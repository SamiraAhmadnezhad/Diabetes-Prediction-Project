...
imputation_values = train_df.groupby('Outcome')[columns_to_impute].transform('mean')
...
# We cannot use grouped imputation on the test set as we don't know its 'Outcome'
# So, we fall back to a robust method: impute with the overall median of the *training* data.
test_imputation_values = X_train.median() # Use median from original (pre-cleaned) train set
